...
@my_log
def create_message_to_server(client_socket, account_name):
...
@my_log
def create_presence_message_to_server(account_name):
    msg = {
        ACTION: PRESENCE,
        TIME: time.time(),
        USER: {
...
@my_log
def get_message_from_server(client_socket, account_name):
    while True:
        try:
...
@my_log
def user_interactive(client_socket, username):
    while True:
        command = input('Введите команду (message, exit): ')
        match command:
            case 'message':
                create_message_to_server(client_socket, username)

            case 'exit':
                send_data(create_exit_message(username), client_socket)
...
def start_client():
    ...
    with socket(AF_INET, SOCK_STREAM) as client_socket:
        try:
            ...
            presence_msg_to_server = create_presence_message_to_server(client_name)
            ...
            clnt_rcvr = Thread(target=get_message_from_server, args=(client_socket, client_name))
